...
def load_embedding_matrix(model_fname):
    model = gensim.models.KeyedVectors.load(model_fname)
    dim = model.vectors.shape[1]
    matrix = np.vstack([np.zeros(dim), model.vectors])
    matrix = np.vstack([matrix, np.random.uniform(-3, 3, size=dim)])
            ...
            while True:
                try:
                    ...
                    if (step > 0) and (step % 200 == 0):
                        print("Step{:4} Mean Loss: {}".format(step, round(np.mean(loss_list), 4)))
